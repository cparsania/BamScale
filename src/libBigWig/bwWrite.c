...
// Deferred parallel block compression. When bwsParallel != 0, data blocks are
// captured (not compressed inline) in flushBuffer and compressed across OpenMP
// threads in batches (bwsFlushBatch, driven from bwFinalize); zoom blocks are
// compressed in parallel in writeZoomLevels. Output is byte-identical to the
...
static const int  **bwsRunV = NULL;   // per-contig run values (RLE, incl. zeros)
static const int  **bwsRunL = NULL;   // per-contig run lengths
static const long  *bwsRunN = NULL;   // per-contig run count
void bwsSetRuns(const int **rv, const int **rl, const long *n) {
    bwsRunV = rv; bwsRunL = rl; bwsRunN = n;
}
typedef struct { unsigned char *raw; unsigned int l, tid, start, end; } bwsBlock_t;
static bwsBlock_t *bwsBuf = NULL;
static unsigned long bwsN = 0, bwsCap = 0;
#ifndef BWS_BATCH
#define BWS_BATCH 16384u   /* blocks per parallel batch (bounds memory) */
#endif
// -----------------------------------------------------------------------------
...
// tid/start/end arrays give each block's index coordinates. 0 on success.
static int bwsWriteBlocks(bigWigFile_t *fp, unsigned char **src, uLongf *srcLen,
    uint32_t *tid0, uint32_t *tid1, uint32_t *bstart, uint32_t *bend, unsigned long n) {
    unsigned long i;
    int err = 0;
    unsigned char **dst;
    uLongf *dstLen;
    ...
    dstLen = malloc(n * sizeof(uLongf));
    if(!dst || !dstLen) { free(dst); free(dstLen); return 1; }
    bound = compressBound(fp->hdr->bufSize);
    for(i = 0; i < n; i++) dst[i] = NULL;
    for(i = 0; i < n; i++) { dst[i] = malloc(bound); if(!dst[i]) err = 1; }
    if(!err) {
#ifdef _OPENMP
        ...
        for(long long j = 0; j < (long long)n; j++) {
            uLongf sz = bound;
            if(compress2(dst[j], &sz, src[j], srcLen[j], bwCompressLevel) != Z_OK) {
#ifdef _OPENMP
    ...
    if(!err) {
        for(i = 0; i < n; i++) {
            long off = bwTell(fp);
            if(fwrite(dst[i], 1, dstLen[i], fp->URL->x.fp) != dstLen[i]) { err = 1; break; }
            if(addIndexEntry(fp, tid0[i], tid1[i], bstart[i], bend[i],
                             (uint64_t)off, (uint64_t)dstLen[i])) { err = 1; break; }
            fp->writeBuffer->nBlocks++;
        }
    }
    for(i = 0; i < n; i++) if(dst[i]) free(dst[i]);
    free(dst); free(dstLen);
    return err;
}

// Flush the currently-buffered deferred data blocks (bwsBuf[0..bwsN)) in parallel.
static int bwsFlushBatch(bigWigFile_t *fp) {
    unsigned long i, n = bwsN;
    int rv;
    unsigned char **src;
    uLongf *srcLen;
    uint32_t *tid, *st, *en;
    ...
    srcLen = malloc(n * sizeof(uLongf));
    tid = malloc(n * sizeof(uint32_t));
    st = malloc(n * sizeof(uint32_t));
    en = malloc(n * sizeof(uint32_t));
    if(!src || !srcLen || !tid || !st || !en) {
        free(src); free(srcLen); free(tid); free(st); free(en);
        return 1;
    }
    for(i = 0; i < n; i++) {
        src[i] = bwsBuf[i].raw; srcLen[i] = bwsBuf[i].l;
        tid[i] = bwsBuf[i].tid; st[i] = bwsBuf[i].start; en[i] = bwsBuf[i].end;
    }
    rv = bwsWriteBlocks(fp, src, srcLen, tid, tid, st, en, n);
    for(i = 0; i < n; i++) free(bwsBuf[i].raw);
    bwsN = 0;
    free(src); free(srcLen); free(tid); free(st); free(en);
    return rv;
}

// Enable/disable deferred parallel compression and reset the batch buffer.
void bwsBeginParallel(int on) {
    unsigned long i;
    if(bwsBuf) { for(i = 0; i < bwsN; i++) if(bwsBuf[i].raw) free(bwsBuf[i].raw); }
    bwsN = 0;
    bwsParallel = on ? 1 : 0;
}
// -----------------------------------------------------------------------------
...
static int flushBuffer(bigWigFile_t *fp) {
    ...
    if(sz) {
        if(bwsParallel) {
            ...
            unsigned char *copy;
            if(bwsN >= bwsCap) {
                unsigned long newcap = bwsCap ? bwsCap * 2 : BWS_BATCH;
                bwsBlock_t *nb = realloc(bwsBuf, newcap * sizeof(bwsBlock_t));
                if(!nb) return 9;
                bwsBuf = nb; bwsCap = newcap;
            }
            copy = malloc(wb->l);
            if(!copy) return 9;
            memcpy(copy, wb->p, wb->l);
            bwsBuf[bwsN].raw = copy; bwsBuf[bwsN].l = wb->l;
            bwsBuf[bwsN].tid = wb->tid; bwsBuf[bwsN].start = wb->start;
            bwsBuf[bwsN].end = wb->end;
            bwsN++;
            wb->l = 24;
...
int constructZoomLevels(bigWigFile_t *fp) {
    ...
    for(i=0; i<fp->cl->nKeys; i++) {
      if(bwsRunV) {
        ...
        long pos = 0, r, nr = bwsRunN[i];
        const int *rv = bwsRunV[i], *rl = bwsRunL[i];
        for(r = 0; r < nr; r++) {
            int v = rv[r];
            long len = rl[r];
            if(v > 0) {
                uint32_t s = (uint32_t)pos, e = (uint32_t)(pos + len);
                float val = (float)v;
                for(k=0;k<fp->hdr->nLevels;k++){
                    if(addIntervalValue(fp, &(fp->writeBuffer->nNodes[k]), sum+k, sumsq+k, fp->writeBuffer->lastZoomBuffer[k], fp->hdr->bufSize/32, fp->hdr->zoomHdrs->level[k], i, s, e, val)) goto error;
...
int writeZoomLevels(bigWigFile_t *fp) {
    ...
    for(i=0; i<fp->hdr->nLevels; i++) {
        ...
        if(bwsParallel) {
            ...
            while(zc) { zn++; zc = zc->next; }
            if(zn) {
                unsigned char **zsrc = malloc(zn * sizeof(unsigned char*));
                uLongf *zlen = malloc(zn * sizeof(uLongf));
                uint32_t *zt0 = malloc(zn * sizeof(uint32_t));
                uint32_t *zt1 = malloc(zn * sizeof(uint32_t));
                uint32_t *zs0 = malloc(zn * sizeof(uint32_t));
                uint32_t *ze0 = malloc(zn * sizeof(uint32_t));
                if(!zsrc || !zlen || !zt0 || !zt1 || !zs0 || !ze0) {
                    free(zsrc); free(zlen); free(zt0); free(zt1); free(zs0); free(ze0);
                    return 2;
                }
                zc = zb;
                while(zc) {
                    uint32_t zlast = (zc->l - 32) >> 2;
                    zsrc[zi] = zc->p; zlen[zi] = zc->l;
                    zt0[zi] = ((uint32_t*)zc->p)[0]; zt1[zi] = ((uint32_t*)zc->p)[zlast];
                    zs0[zi] = ((uint32_t*)zc->p)[1]; ze0[zi] = ((uint32_t*)zc->p)[zlast+2];
                    zi++; zc = zc->next;
                }
                if(bwsWriteBlocks(fp, zsrc, zlen, zt0, zt1, zs0, ze0, zn)) {
                    free(zsrc); free(zlen); free(zt0); free(zt1); free(zs0); free(ze0);
                    return 2;
                }
                free(zsrc); free(zlen); free(zt0); free(zt1); free(zs0); free(ze0);
        ...
        while(zb) {
            sz = fp->hdr->bufSize;
            if(compress2(wb->compressP, &sz, zb->p, zb->l, bwCompressLevel) != Z_OK) return 2;
...
int bwFinalize(bigWigFile_t *fp) {
    ...
    if(fp->hdr->nLevels && fp->writeBuffer->nBlocks) {
        struct timespec _ta, _tb, _tc;
        if(bwsVerbose) clock_gettime(CLOCK_MONOTONIC, &_ta);
        offset = bwTell(fp);
        if(makeZoomLevels(fp)) return 5;
        if(constructZoomLevels(fp)) return 6;
        if(bwsVerbose) clock_gettime(CLOCK_MONOTONIC, &_tb);
        bwSetPos(fp, offset);
