...
CURLcode urlFetchData(URL_t *URL, unsigned long bufSize) {
    ...
    if(rv != CURLE_OK) {
        REprintf("[urlFetchData] Couldn't set the range (%s)\n", range);
        return rv;
    }
...
size_t url_fread(void *obuf, size_t obufSize, URL_t *URL) {
    ...
    while(remaining) {
        if(!URL->bufLen) {
            ...
            if(rv != CURLE_OK) {
                REprintf("[url_fread] urlFetchData (A) returned %s\n", curl_easy_strerror(rv));
                return 0;
            }  
            ...
            if(remaining) {
                ...
                if(rv != CURLE_OK) {
                    REprintf("[url_fread] urlFetchData (B) returned %s\n", curl_easy_strerror(rv));
                    return 0;
                }
            }
        } else {
...
CURLcode urlSeek(URL_t *URL, size_t pos) {
    ...
    if(URL->type == BWG_FILE) {
        ...
        if(pos < URL->filePos || pos >= URL->filePos+URL->bufLen) {
            ...
            rv = curl_easy_setopt(URL->x.curl, CURLOPT_RANGE, range);
            if(rv != CURLE_OK) {
                REprintf("[urlSeek] Couldn't set the range (%s)\n", range);
                return rv;
            }
            rv = curl_easy_perform(URL->x.curl);
            if(rv != CURLE_OK) {
                REprintf("[urlSeek] curl_easy_perform received an error!\n");
            }
...
URL_t *urlOpen(const char *fname, CURLcode (*callBack)(CURL*), const char *mode) {
    ...
    if((!mode) || (strchr(mode, 'w') == 0)) {
        ...
        if(URL->type == BWG_FILE) {
            ...
            if(!(URL->x.fp)) {
                free(URL);
                REprintf("[urlOpen] Couldn't open %s for reading\n", fname);
                return NULL;
            }
#ifndef NOCURL
        } else {
            ...
            if(!(URL->memBuf)) {
                free(URL);
                REprintf("[urlOpen] Couldn't allocate enough space for the file buffer!\n");
                return NULL;
            }
            URL->bufSize = GLOBAL_DEFAULTBUFFERSIZE;
            URL->x.curl = curl_easy_init();
            if(!(URL->x.curl)) {
                REprintf("[urlOpen] curl_easy_init() failed!\n");
                goto error;
            }
            //Negotiate a reasonable HTTP authentication method
            if(curl_easy_setopt(URL->x.curl, CURLOPT_HTTPAUTH, CURLAUTH_ANY) != CURLE_OK) {
                REprintf("[urlOpen] Failed instructing curl to use any HTTP authentication it finds to be suitable!\n");
                goto error;
            }
            //Follow redirects
            if(curl_easy_setopt(URL->x.curl, CURLOPT_FOLLOWLOCATION, 1L) != CURLE_OK) {
                REprintf("[urlOpen] Failed instructing curl to follow redirects!\n");
                goto error;
            }
            //Set the URL
            if(curl_easy_setopt(URL->x.curl, CURLOPT_URL, fname) != CURLE_OK) {
                REprintf("[urlOpen] Couldn't set CURLOPT_URL!\n");
                goto error;
            }
            ...
            if(curl_easy_setopt(URL->x.curl, CURLOPT_RANGE, range) != CURLE_OK) {
                REprintf("[urlOpen] Couldn't set CURLOPT_RANGE (%s)!\n", range);
                goto error;
            }
            ...
            if(curl_easy_setopt(URL->x.curl, CURLOPT_WRITEFUNCTION, bwFillBuffer) != CURLE_OK) {
                REprintf("[urlOpen] Couldn't set CURLOPT_WRITEFUNCTION!\n");
                goto error;
            }
            if(curl_easy_setopt(URL->x.curl, CURLOPT_WRITEDATA, (void*)URL) != CURLE_OK) {
                REprintf("[urlOpen] Couldn't set CURLOPT_WRITEDATA!\n");
                goto error;
            }
            ...
            if(curl_easy_setopt(URL->x.curl, CURLOPT_SSL_VERIFYPEER, 0) != CURLE_OK) {
                REprintf("[urlOpen] Couldn't set CURLOPT_SSL_VERIFYPEER to 0!\n");
                goto error;
            }
            if(curl_easy_setopt(URL->x.curl, CURLOPT_SSL_VERIFYHOST, 0) != CURLE_OK) {
                REprintf("[urlOpen] Couldn't set CURLOPT_SSL_VERIFYHOST to 0!\n");
                goto error;
            }
            if(callBack) {
                code = callBack(URL->x.curl);
                if(code != CURLE_OK) {
                    REprintf("[urlOpen] The user-supplied call back function returned an error: %s\n", curl_easy_strerror(code));
                    goto error;
                }
            }
            ...
            errno = 0; //Sometimes curl_easy_perform leaves a random errno remnant
            if(code != CURLE_OK) {
                REprintf("[urlOpen] curl_easy_perform received an error: %s\n", curl_easy_strerror(code));
                goto error;
            }
#endif
        }
        ...
        if(!(URL->x.fp)) {
            free(URL);
            REprintf("[urlOpen] Couldn't open %s for writing\n", fname);
            return NULL;
        }
    }
    if(url) free(url);
