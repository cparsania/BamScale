...
error:
    bwHdrDestroy(bw->hdr);
    REprintf("[bwHdrRead] There was an error while reading in the header!\n");
    bw->hdr = NULL;
}
...
void bwClose(bigWigFile_t *fp) {
    if(!fp) return;
    if(bwFinalize(fp)) {
        REprintf("[bwClose] There was an error while finishing writing a bigWig file! The output is likely truncated.\n");
    }
...
error:
    if(o) free(o);
    REprintf("Got an error in bbGetSQL!\n");
    return NULL;
}
...
bigWigFile_t *bwOpen(const char *fname, CURLcode (*callBack) (CURL*), const char *mode) {
    ...
    if(!bwg) {
        REprintf("[bwOpen] Couldn't allocate space to create the output object!\n");
        return NULL;
    }
    if((!mode) || (strchr(mode, 'w') == NULL)) {
        ...
        if(!bwg->URL) {
            REprintf("[bwOpen] urlOpen is NULL!\n");
            goto error;
        }
        ...
        bwHdrRead(bwg);
        if(!bwg->hdr) {
            REprintf("[bwOpen] bwg->hdr is NULL!\n");
            goto error;
        }
        ...
        if(!bwg->cl) {
            REprintf("[bwOpen] bwg->cl is NULL (%s)!\n", fname);
            goto error;
        }
        ...
        if(bwg->hdr->indexOffset) {
            ...
            if(!bwg->idx) {
                REprintf("[bwOpen] bwg->idx is NULL bwg->hdr->dataOffset 0x%"PRIx64"!\n", bwg->hdr->dataOffset);
                goto error;
            }
        }
    } else {
...
bigWigFile_t *bbOpen(const char *fname, CURLcode (*callBack) (CURL*)) {
    ...
    if(!bb) {
        REprintf("[bbOpen] Couldn't allocate space to create the output object!\n");
        return NULL;
    }
